// GBN 世界地图 · 数据层
// 地图用两层 ASCII 图 authoring：归属层 + 地形层。每行 16 字符，共 10 行。
...
export const TERRAIN: Record<TerrainKey, Terrain> = {
...
export const RESOURCE_ORDER: Resource[] = ['耗材', '木材', '矿石', '港口', '冷凝']
...
export const AI_POOL: Record<DraftKind, AiDraft[]> = {
  terrain: [
...
export const nationById = (id: string | null): Nation | undefined =>
  NATIONS.find(n => n.id === id)

export const nationByGlyph = (g: string): Nation | undefined =>
  NATIONS.find(n => n.glyph === g)
...
/** 从两张 ASCII 图构建格子表。海洋格不入表。 */
export function buildCells(): Map<string, import('./types.js').Cell> {
  const cells = new Map<string, import('./types.js').Cell>()
  for (let r = 0; r < MAP_H; r++) {
    for (let c = 0; c < MAP_W; c++) {
      const og = OWNER_MAP[r][c]
      const tg = TERRAIN_MAP[r][c] as TerrainKey
      if (og === '.') continue                       // 海洋
      cells.set(cellKey(c, r), {
        col: c, row: r,
        owner: og === '*' ? null : (nationByGlyph(og)?.id ?? null),
