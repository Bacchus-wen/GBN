...
## 一、地形调参（✅ 已完成，见分支 `feat/terrain-tuning`）

三项均已修复并有回归测试守住。原始记录保留在下方以备追溯。

| 项 | 处置 |
|---|---|
| 噪声层各向异性 | 噪声域坐标改为按世界长宽比缩放（几何平均作参考长度）。8 种子平均坡度比 p/f/m/i 全部落在 0.88~1.14，修正前为 1.67~1.91。已加 `it.each` 回归测试 |
| 边界软化各向异性 | `buildMasks` 增加可选的按轴 σ；烘焙侧以世界单位（3.5）定义过渡带宽度，再按轴换算成像素 σ |
| 垂直起伏偏弱 | 新增显式的 `verticalScale` 参数（烘焙用 2.4×），而非偷改 `profiles.base` 让图例说谎。高度值域 32.8 → 84.4 |
| 相机取景 | 改为精确拟合包围盒八个角点（原先用包围球，对 400×84×250 的扁盒子过于保守）。并修好了画布高度塌陷 |
| 海平面（本轮新增） | 补了半透明海平面。此前海洋区只是同色低地，整块世界读起来是悬空矩形板，看不出海岸线 |

### 本轮发现的既有缺陷

`.map-canvas { height: 520px }` 这条 CSS 一直存在且是为地形画布准备的，但 Task 8 接线时
没有用上，且 R3F 的 `<Canvas>` 会在包裹 div 上打**内联**的 `height: 100%`，内联优先级高于类，
所以即使套上类也无效。高度必须给在父级 `.map-stage` 上。修正前画布只有约 160px 高。

---

## 一（原始记录）、已裁定并入「地形调参」后续计划
